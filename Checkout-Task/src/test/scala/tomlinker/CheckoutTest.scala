...
class CheckoutTest extends org.scalatest.FunSuite {
  ...
  test("format_order test") {
    ...
    val expected_output =
      List(ItemOrder("A", 5), ItemOrder("D", 3), ItemOrder("C", 4), ItemOrder("B", 2))
  ...
  test("format_prices test") {
    ...
    val expected_output =
      List(ItemPrice("A", 40, Some(Deal(4, 100))), ItemPrice("Jeremy784", 34.5, None))
  ...
  test("get_item_price_from_list test - acceptance and rejection") {
    ...
    assert(
      get_item_price_from_list(";'sdf", prices) == Some(ItemPrice(";'sdf", 50, Some(Deal(2, 95)))))
